...
#       （实测坏实现可达 121642 次）。
#
#   [电平语义三段 + 队列可读性]  未消费时连续 5 次 poll 都应报可读
#       （one-shot 实现只能通过 1 次）；消费后必须立刻变 0；新开的 fd 在
#       "缓冲确有未读样本"时必须立刻可读。
#       注：阶段 03 引入 kfifo 后，可读性由"共享队列是否非空"决定（原 per-fd
#       last_seq 方案在多进程共享队列时会谎报可读）。断言强度不变，只是前提由
#       "设备曾产生过样本"变成"此刻仍有未读样本"，该前提由
#       user/io_models_test.c 静置两个采样周期来确定性建立。
#
