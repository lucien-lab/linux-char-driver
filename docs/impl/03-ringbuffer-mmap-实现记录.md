...
- **验证日志（整改后，当前有效）**：
  - 03 阶段稳定性连跑 3 次均 **40 PASS / 0 FAIL / 0 SKIP**：
    `logs/20260914-141113-03v-03-ringbuffer-mmap.log`、
    `logs/20260914-141125-03v-03-ringbuffer-mmap.log`、
    `logs/20260914-141137-03v-03-ringbuffer-mmap.log`
    （三轮 `[TEST:END]` 均为 `pass=40 fail=0 skip=0`）
  - 回归（同一份最终产物）：`logs/20260914-141208-03v-01-io-models.log`（16 PASS / 0 FAIL）、
    `logs/20260914-141220-03v-smoke.log`（15 PASS / 0 FAIL）
- **测试结论**：03-ringbuffer-mmap **40 PASS / 0 FAIL**（连跑 3 次无抖动）；01-io-models **16 PASS / 0 FAIL**；
  smoke **15 PASS / 0 FAIL**（无 `WARNING:`/`Call trace:`，无 panic）
- **历史日志（整改前，仅作对比）**：`logs/20260914-111209-03-ringbuffer-mmap.log`（当时脚本 28 项）；
  那一轮引用的放大器参数（20ms 窗口 / 15s 观测）已被本文第九节取代，参数与项数请以第九节为准。
...
- **为什么没被测试抓住**：配套检查项只断言"`mmap_invalid == 0`"，而
  ...
  让用户态重试条件永假（NC-A），**当时 28 项的脚本仍然 28 PASS / 0 FAIL**。
  （整改后由复验者重做：NC-a 得 33 PASS / 7 FAIL，确定性探针 0→0 被抓住 ——
   说明新增的"等一次 seq 变化"探针确实把这类假实现变成了可抓，
   日志 `logs/20260914-134626-03v-03-ringbuffer-mmap.log`。）
...
**【整改关键】放大器延时必须是睡眠而不是自旋**：
初版在 `spinlock` 内用 `udelay()` 自旋——写者是 RT 优先级的中断线程，自旋期间会占满一个 CPU；
读者若被调度到同一 CPU，**在整个窗口内得不到执行机会**，于是"永远观测不到奇数值"。
实测症状：同一代码连跑 3 次有 2 次 `odd_seen=0`（37/3、40/0、37/3），并触发 `sched: RT throttling`。
改成 `fsleep()`（睡眠延时）后写者在窗口内主动让出 CPU，观测变得确定可复现，连跑 3 次均 40/0。
配套改变：`shm_lock` 从 `spinlock_t` 换成 `mutex`（两个写入点都在可睡眠上下文）。

当前配置：`shm_publish_delay_us=1000`（`fsleep(1000)` 实际 1~2ms，占空比 10~20%），观测 2 秒：

| 观测项 | 正常模式 | 放大器模式（1ms 睡眠窗口，2s 观测） |
|---|---|---|
| 快照次数 | 100 | 431 万 ~ 436 万 |
| 撞上写入窗口次数（`shm_seq_odd_seen`） | 0 | 159 ~ 200 |
| 重试次数（`mmap_retries`） | 0 | 2.96 亿 ~ 3.52 亿（`long long`，不再溢出） |
| 撕裂样本（`mmap_invalid`） | 0 | **0** |
| 共享区 `seq` 是否推进（确定性探针） | 是 | 是（`shm_seq_changes` 358~400） |

**【SKIP 语义（整改新增）】**若因宿主调度原因本次没撞上窗口（`odd_seen=0`），
放大器相关 4 项不再硬失败、也不再假装通过，而是输出 `[CHECK:SKIP] <名称> -- <原因>`：
SKIP 表示"本次没取得证据"，在 `[TEST:END]` 与运行器结论里单独计数并列出理由。
这样即使环境变化，结果仍然稳定（fail 恒为 0）且诚实（不把未观测当成已验证）。
...
> 注：上面这段是**整改前**（28 项脚本）的输出快照，仅作历史对比；
> 当前有效结论（40 项、连跑 3 次）见本文第九节「复验反馈整改记录」。
...
（该项已由整改后的 40 项脚本重新验证：连跑 3 次均 40 PASS / 0 FAIL，见第九节。）
...
复验者判定阶段 03「不通过」，给了两个 blocker（N1/N3）与两个文档问题（N2/N4），
外加两条低危发现（N5/N6）。本节记录逐条整改与验证证据。
完整复验报告：`docs/verify/03-ringbuffer-mmap-复验报告.md`。

### 9.1 N1（blocker）：放大器检查间歇失败 → 根因是"自旋放大窗口"

| 项 | 内容 |
|---|---|
| 现象 | 同一代码连跑 3 次 = 37/3、40/0、37/3；失败固定在放大器项（`iters≈5.1e6, seq_changes≈200, odd_seen=0, retries=0`）；伴随 `sched: RT throttling` |
| 根因 | 放大器在 `spinlock` 内用 `udelay()` **自旋**。写者是 RT 优先级的中断线程，自旋期间占满一个 CPU；读者若被调度到同一 CPU，在整个窗口内得不到执行机会 → 永远观测不到奇数值。能否命中取决于宿主调度运气 |
| 修法 | ① `sensor_shm_publish()` 的放大延时由 `udelay()` 改为 `fsleep()`（**睡眠**延时，窗口内主动让出 CPU）；② 配套把 `shm_lock` 从 `spinlock_t` 换成 `mutex`（两个写入点都在可睡眠上下文：线程化下半部、ioctl 进程上下文）；③ 窗口参数取 1ms（占空比 10~20%），重试上限 20 万，观测 2s；④ 放大器相关检查项加 `[CHECK:SKIP]` 语义（见 9.2） |
| 验证 | 连跑 3 次均 **40 PASS / 0 FAIL / 0 SKIP**：`logs/20260914-141113-03v-03-ringbuffer-mmap.log`、`-141125-`、`-141137-` |
| 实测 | `iters=4.31~4.36M`、`odd_seen=159~198`、`retries=2.96~3.22 亿`、`invalid=0`（修前：odd_seen 常为 0） |

**过程中的两个反面教训（都实际踩到并记录了）**：
1. 第一版试过把窗口放到 5ms（`fsleep(5000)` 实际 5~10ms，接近 100% 占空比）并把重试上限提到 200 万
   → 单次快照长时间自旋、测试直接挂死（600s 超时）。**放大器参数必须让重试次数有界**：
   "窗口时长 × 读者速度 < 重试上限" 是硬约束。
2. 若只把 `udelay` 换成 `fsleep` 而不同时把 `spinlock` 换成 `mutex`，就是"持自旋锁睡眠"——
   属于内核非法用法。两者必须一起改。

### 9.2 SKIP 语义（新增的诚实表达机制）

`tests/runner/lib.sh` 新增 `skip "名称" "原因"`，输出 `[CHECK:SKIP] <名称> -- <原因>`；
`init.sh` 的总结行扩为 `[TEST:END] <阶段> pass=N fail=M skip=K`；
`scripts/22-macos-run-test.sh` 单独统计并在结论与明细里列出 SKIP 及其理由（**SKIP 不算失败**）。

纪律：SKIP 只用于"本次环境没造出可观测条件"，**不允许**用它掩盖功能缺陷
（功能坏了必须 FAIL）。放大器 4 项在 `odd_seen==0` 时降级为 SKIP，
其余 36 项无论环境如何都必须 PASS/FAIL。

### 9.3 N3（blocker）：出队全局唯一性断言无判别力

- 复验负控 NC-c：把 read 侧改成**无锁** `kfifo_out`（真实出队竞态），测试仍 40/0
  → 该断言对"多读者出队原子性"**没有判别力**（TCG 下真并行度受宿主调度限制，竞态轻易不出现）。
- 整改：检查项**改名**为「本次运行未观测到重复样本（dup_seqs=0；冒烟级不变式，非原子性证明）」，
  并在脚本注释与文档里明确：**不得**把它当作"无竞态"的证据。
- 「无竞态」的证据等级改由三项组成（见 9.5），其中该断言只作为"冒烟级不变式"列在最后。

### 9.4 N2/N4：文档与代码/日志不一致

- 文件头部的日志引用已从「28 项旧日志」改为当前「40 项、连跑 3 次」的实测日志（见文首）。
- §3.2.3 的参数表已从「20ms 窗口 / 15s 观测」改为当前的「1ms 睡眠窗口 / 2s 观测」，
  并附上实测区间（426~445 万次快照、`odd_seen` 161~200、重试 2.9~3.5 亿）。
- §7 里那段 `pass=28` 的输出快照已加注"整改前历史对比"。
- 复验者指出的 N5（放大参数会把写者临界区拉长、进而连带持有 `sd->lock`）已在代码注释里说明；
  N6（rmmod 与活跃 mmap 并存的 UAF 风险未测）保留在第八节遗留风险里，未做无证据的断言。

### 9.5 当前「无竞态」的诚实表述（可直接引用）

> 阶段 03 的并发正确性采用**三重证据**：① QEMU MTTCG 双核（`-smp 2 -accel tcg,thread=multi`）下
> 8 进程 × 200 轮并发回归全绿（`failed=0 / invalid_samples=0 / i2c_errors=0`），
> 并附「样本守恒 `ring_count + dropped == irq_count`」不变量；
> ② mmap 共享区实现**用户态可见**的 seqlock（内核维护奇/偶 `seq` + 双侧 `smp_wmb`），
> 用睡眠式受控放大器拿到「重试路径真实执行（重试 2.9~3.5 亿次）且撕裂样本为 0」的正向证据，
> 且**负控可证伪**（删写侧 `seq` 自增 → 探针 0→0、7 项 FAIL；去掉读侧重试 + 强放大 → `invalid=39`）；
> ③ `CONFIG_KCSAN_STRICT` 内核下同一负载零 data-race 报告
> （唯一报告位于 `mm/rmap` 进程 exec/exit 路径，报告块内无任何驱动符号，与驱动无关）。
>
> **边界（必须一并说明）**：以上是「高置信度证据」，**不等于数学证明**；
> TCG 的并行度受宿主调度影响；「出队全局唯一性」断言在负控下未能触发，
> 只能表述为"本次运行未观测到重复样本"。真正的并发保证仍应以 KCSAN/lockdep 与真机 SMP 压测为准。

### 9.6 复验反馈的最终状态

| 编号 | 严重度 | 状态 | 证据 |
|---|---|---|---|
| N1 放大器间歇失败 | 高（blocker） | **已修** | 连跑 3 次 40/0/0；根因=自旋窗口，修法=睡眠延时+mutex |
| N3 唯一性断言无判别力 | 中（blocker） | **已如实降级** | 改名+注释+本文 9.3/9.5，不再作为并发证据 |
| N2 文档参数与脚本不符 | 中 | **已修** | 文首日志、§3.2.3 参数表、§7 注释 |
| N4 头部引用旧日志 | 低 | **已修** | 文首改为当前 3 次日志 |
| N5 放大参数拉长临界区 | 低 | **已说明** | 代码注释（`sensor_shm_publish` 与模块参数说明） |
| N6 rmmod×活跃 mmap 的 UAF | 低 | **保留为遗留风险** | 第八节；未做无证据断言 |
